...
export default async function NuevaComisionPage() {
  ...
  requireRoleOrRedirect(me.roles, ["ADMIN", "RRHH", "SUPERVISOR", "INSPECTOR"]);
  const funcs = await api.get<Page<Func>>("/funcionarios?page_size=200&estatus=ACTIVO", token).catch(() => ({ items: [] as Func[], total: 0 }));
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        <Link href="/ops/comisiones" className="text-xs text-muted-foreground hover:underline">← Comisiones</Link>
        <h1 className="text-2xl font-bold mt-1">Nueva comisión</h1>
      </div>
