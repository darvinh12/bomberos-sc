...
import { crearFalta, type State } from "./actions";
...
function Submit() {
  ...
  return (
    <button type="submit" disabled={pending} className="rounded-md bg-primary text-primary-foreground px-5 py-2.5 font-semibold hover:opacity-90 disabled:opacity-60">
      {pending ? "Guardando…" : "Registrar falta"}
    </button>
  );
}

export default function Form({ funcionarios }: { funcionarios: Func[] }) {
  const [state, action] = useFormState(crearFalta, initial);
  return (
    <form action={action} className="rounded-xl border bg-card p-6 space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <select name="tipo_falta" required defaultValue="LEVE" className="input">
      ...
      <div>
        ...
        <textarea name="descripcion" required rows={3} minLength={3} className="input" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium mb-1">Inicio suspensión</label>
          <input type="date" name="fecha_inicio_susp" className="input" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1">Fin suspensión</label>
          <input type="date" name="fecha_fin_susp" className="input" />
        </div>
      </div>
      <div className="flex justify-between pt-4 border-t">
        <Link href="/ops/faltas" className="rounded-md border border-input px-4 py-2 text-sm font-medium hover:bg-accent">Cancelar</Link>
        <Submit />
      </div>
    </form>
  );
