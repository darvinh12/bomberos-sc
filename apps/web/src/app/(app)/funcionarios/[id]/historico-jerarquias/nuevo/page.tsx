...
import NuevoHistJerarquiaForm from "./form";
...
export default async function NuevoHistJerarquiaPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  const jerarquias = await api
    .get<Catalogo[]>("/catalogos/jerarquias", token)
    .catch(() => [] as Catalogo[]);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold mt-1">Nuevo registro de jerarquía</h1>
        <p className="text-sm text-muted-foreground">
          Para: <span className="font-semibold">{nombre}</span>
        </p>
      </div>
      <NuevoHistJerarquiaForm funcionarioId={fid} jerarquias={jerarquias} />
    </div>
  );
}
