...
import NuevaHabilidadForm from "./form";
...
export default async function NuevaHabilidadPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  if (!puedeEditarSeccion("habilidades", me.roles)) redirect(`/funcionarios/${fid}`);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold mt-1">Nueva habilidad</h1>
        <p className="text-sm text-muted-foreground">
          Para: <span className="font-semibold">{nombre}</span>
        </p>
      </div>
      <NuevaHabilidadForm funcionarioId={fid} />
    </div>
  );
}
