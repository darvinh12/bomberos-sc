...
import { crearRadio, type State } from "./actions";
...
function Submit() {
  ...
  return (
    <button type="submit" disabled={pending} className="rounded-md bg-primary text-primary-foreground px-5 py-2.5 font-semibold hover:opacity-90 disabled:opacity-60">
      {pending ? "Guardando…" : "Crear radio"}
    </button>
  );
}

export default function Form({ estaciones }: { estaciones: Cat[] }) {
  const [state, action] = useFormState(crearRadio, initial);
  return (
    <form action={action} className="rounded-xl border bg-card p-6 space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium mb-1">Modelo (id) *</label>
          <input type="number" name="modelo_id" required min="1" defaultValue="1" className="input" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1">Serial *</label>
          <input name="serial" required minLength={3} className="input" />
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-xs font-medium mb-1">Placa inventario</label>
          <input name="placa_inv" className="input" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1">Frecuencia</label>
          <input name="frecuencia" className="input" placeholder="ej. 154.500" />
        </div>
        <div>
          <label className="block text-xs font-medium mb-1">Canal</label>
          <input name="canal" className="input" />
        </div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
