...
import { crearReposo, type State } from "./actions";
...
function Submit() {
  ...
  return (
    <button type="submit" disabled={pending} className="rounded-md bg-primary text-primary-foreground px-5 py-2.5 font-semibold hover:opacity-90 disabled:opacity-60">
      {pending ? "Guardando…" : "Crear reposo"}
    </button>
  );
}

export default function Form({ funcionarios }: { funcionarios: Func[] }) {
  const [state, action] = useFormState(crearReposo, initial);
  return (
    <form action={action} className="rounded-xl border bg-card p-6 space-y-5">
      {state.error && (
        <div className="rounded-md bg-destructive/10 border border-destructive/30 p-3 text-sm text-destructive">{state.error}</div>
      )}
      <div>
        ...
        <select name="funcionario_id" required defaultValue="" className="input">
          ...
          {funcionarios.map((f) => (
            <option key={f.id} value={f.id}>
              {f.nombre_completo ?? `${f.apellidos}, ${f.nombres}`} ({f.nacionalidad}-{f.cedula})
            </option>
          ))}
        </select>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <select name="tipo_reposo_id" required defaultValue="1" className="input">
            ...
            <option value="2">Postoperatorio</option>
      ...
      <div className="flex justify-between pt-4 border-t">
        <Link href="/salud/reposos" className="rounded-md border border-input px-4 py-2 text-sm font-medium hover:bg-accent">Cancelar</Link>
        <Submit />
      </div>
    </form>
  );
